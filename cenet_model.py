...
class Oracle(nn.Module):
    ...
    def __init__(self, input_dim, out_dim):
        ...
        self.linear = nn.Sequential(
            nn.Linear(input_dim, input_dim),  # 线性层
            nn.BatchNorm1d(input_dim),  # BN层，用于加速收敛
            nn.Dropout(0.4),  # Dropout层，防止过拟合
            nn.LeakyReLU(0.2),  # 激活函数
            nn.Linear(input_dim, out_dim),  # 线性层
        )
...
class CENET(nn.Module):
    ...
    def __init__(self, num_e, num_rel, num_t, args):
        ...
        self.linear_pred_layer_s1 = nn.Linear(
            2 * args.embedding_dim, args.embedding_dim
        )
        self.linear_pred_layer_o1 = nn.Linear(
            2 * args.embedding_dim, args.embedding_dim
        )

        self.linear_pred_layer_s2 = nn.Linear(
            2 * args.embedding_dim, args.embedding_dim
        )
        self.linear_pred_layer_o2 = nn.Linear(
            2 * args.embedding_dim, args.embedding_dim
        )
    ...
    def forward(self, batch_block, mode_lk, total_data=None):
        ...
        if mode_lk == "Training":
            s_nce_loss, _ = self.calculate_nce_loss(
                s,
                o,
                r,
                self.rel_embeds[: self.num_rel],
                self.linear_pred_layer_s1,
                self.linear_pred_layer_s2,
                s_history_tag,
                s_non_history_tag,
            )
            o_nce_loss, _ = self.calculate_nce_loss(
                o,
                s,
                r,
                self.rel_embeds[self.num_rel:],
            ...
            s_spc_loss = self.calculate_spc_loss(
                s,
                r,
                self.rel_embeds[: self.num_rel],
                s_history_label_true,
            ...
            s_ce_loss, s_pred_history_label, s_ce_all_acc = self.oracle_loss(
                s,
                r,
                self.rel_embeds[: self.num_rel],
                s_history_label_true,
            ...
            s_total_loss1, sub_rank1 = self.link_predict(
                s_nce_loss, s_preds, s_ce_loss, s, o, r, s_mask, total_data, "s", True
            )
            o_total_loss1, obj_rank1 = self.link_predict(
                o_nce_loss, o_preds, o_ce_loss, o, s, r, o_mask, total_data, "o", True
            )
            batch_loss1 = (s_total_loss1 + o_total_loss1) / 2.0

            s_total_loss2, sub_rank2 = self.link_predict(
                s_nce_loss, s_preds, s_ce_loss, s, o, r, s_mask, total_data, "s", False
            )
            o_total_loss2, obj_rank2 = self.link_predict(
                o_nce_loss, o_preds, o_ce_loss, o, s, r, o_mask, total_data, "o", False
            )
            ...
            s_total_loss3, sub_rank3 = self.link_predict(
                s_nce_loss,
                s_preds,
                s_ce_loss,
                s,
            ...
            o_total_loss3, obj_rank3 = self.link_predict(
                o_nce_loss,
                o_preds,
                o_ce_loss,
                o,
            ...
            s_ce_loss, _, _ = self.oracle_loss(
                s,
                r,
                self.rel_embeds[: self.num_rel],
                s_history_label_true,
    ...
    def oracle_loss(self, actor1, r, rel_embeds, history_label, frequency_hidden):
        history_label_pred = F.sigmoid(
            self.oracle_layer(
                torch.cat(
                    (self.entity_embeds[actor1], rel_embeds[r], frequency_hidden), dim=1
                )
            )
        )
        ...
        ce_accuracy = 1.0 * ce_correct.item() / tmp_label.shape[0]
        print("# CE Accuracy", ce_accuracy)
        ce_loss = self.crossEntropy(
            torch.squeeze(history_label_pred), torch.squeeze(history_label)
        )
    ...
    def calculate_nce_loss(
        ...
        preds_raw1 = self.tanh(
            linear1(
                self.dropout(
                    torch.cat((self.entity_embeds[actor1], rel_embeds[r]), dim=1)
                )
            )
        )
        preds1 = F.softmax(
            preds_raw1.mm(self.entity_embeds.transpose(0, 1)) + history_tag, dim=1
        )

        preds_raw2 = self.tanh(
            linear2(
                self.dropout(
                    torch.cat((self.entity_embeds[actor1], rel_embeds[r]), dim=1)
                )
        ...
        preds2 = F.softmax(
            preds_raw2.mm(self.entity_embeds.transpose(0, 1)) + non_history_tag, dim=1
        )
    ...
    def regularization_loss(self, reg_param):
        regularization_loss = torch.mean(self.rel_embeds.pow(2)) + torch.mean(
            self.entity_embeds.pow(2)
        )
        return regularization_loss * reg_param

    # 计算 Oracle 层的 L1 正则化损失
